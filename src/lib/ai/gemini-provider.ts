...
export class GeminiProvider implements AIProvider {
  ...
  async analyzeProduct(input: ProductAnalysisInput): Promise<ProductAnalysisResult> {
    const system = `Kamu adalah Gemini AI Commerce Analyst. Analisis produk dan hasilkan format JSON persis sesuai schema berikut:
{
    ...
    const system = `Buat prompt JSON video AI siap pakai:
{
  ...
  async analyzePerformance(data: PerformanceData[]): Promise<PerformanceAnalysisResult> {
    const system = `Kamu adalah Gemini AI Analyst. Analisis data performa affiliate dan hasilkan JSON:
{
  ...
  async generateNextContent(
    ...
    try {
      const system = `Kamu adalah Gemini Decision Engine. Hasilkan 5 rekomendasi konsep konten berikutnya dalam format JSON:
{
  "concepts": [
    {
      "title": string,
  ...
  async generateInsights(
    ...
    return {
      ...
      why: [
        "Audiens media sosial membutuhkan demonstrasi visual fungsi nyata.",
  ...
  async generateRecommendations(data: PerformanceData[]): Promise<RecommendationResult> {
    ...
    return {
      ...
      summary: "Evaluasi performa Gemini Live AI",
    };
  }
}
